...
def class_tests():
    ...
    blockchain.add_block_toChain('address_x')
    print("\nAfter validating block (in bitcoin word mining) . . .")
    print("Length of Updated blockchain is: {}".format(len(blockchain.chain)))
    print_blockchain(blockchain.chain)

    blockchain.add_block_toChain('address_y')
    print("\nAfter One more validating block (in bitcoin word mining) . . .")
    print("Length of Updated blockchain is: {}".format(len(blockchain.chain)))
    print_blockchain(blockchain.chain)
...
#in bitcoin currency it is called mining
def add_block_toChain(server):
    resp = requests.get(server + '/validate_Block').json()
    print("On Server {}: Block has been added (or mined) successfully!\n".format(server))
    return resp
...
def api_tests():
    ...
    add_block_toChain(server2)  # validated and Added ( or Mined) a new block on server2
